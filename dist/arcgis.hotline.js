define([ "esri/layers/Layer", "esri/geometry/Point", "esri/geometry/support/webMercatorUtils", "esri/views/2d/layers/BaseLayerView2D" ], function(Layer, Point, webMercatorUtils, BaseLayerView2D) {
    this.min = 0;
    this.max = 1;
    this.palette = {};
    ...
    const getRGBForValue = value => {
        var valueRelative = Math.min(Math.max((value - this.min) / (this.max - this.min), 0), .999);
        var paletteIndex = Math.floor(valueRelative * 256) * 4;
        return [ this.palette[paletteIndex], this.palette[paletteIndex + 1], this.palette[paletteIndex + 2] ];
    };
    return HotlineLayer;
});
